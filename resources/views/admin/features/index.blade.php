...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-lg font-semibold text-gray-900">Özellik Listesi</h2><a href="{{ route('admin.features.create') }}"
    ...
    <div class="bg-white rounded-xl shadow-sm overflow-hidden">
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">@forelse($features as $f)<tr class="hover:bg-gray-50">
                <td class="px-6 py-4">
                    <div class="font-medium text-gray-900">{{ $f->title }}</div>
                    <div class="text-sm text-gray-500">{{ Str::limit($f->description, 60) }}</div>
                </td>
                ...
                <td class="px-6 py-4 text-right space-x-2"><a href="{{ route('admin.features.edit', $f) }}"
                        class="text-blue-600">Düzenle</a>
                    <form action="{{ route('admin.features.destroy', $f) }}" method="POST" class="inline"
                        onsubmit="return confirm('Sil?')">@csrf @method('DELETE')<button
                            class="text-red-600">Sil</button></form>
                </td>
            </tr>@empty<tr>
                    <td colspan="4" class="px-6 py-12 text-center text-gray-500">Henüz özellik yok.</td>
                </tr>@endforelse</tbody>
        </table>
    </div>
@endsection